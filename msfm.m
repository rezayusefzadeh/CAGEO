...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Perm=load('Perm-het.txt');  %%%%%%%%%% Loading Pemeability Values
% Poro=load('Poro.txt');  %%%%%%%%%% Loading Porosity Values 
% Perm=20*ones(60,220);
% Poro=0.15*ones(60,220);
% 
% ct=4.95E-6;                %%%%%%%%%% Total Compressibility Value
% mio=0.88;                  %%%%%%%%%% Viscosity
% dx=50;
% dy=50;
% F=(1/dx)*sqrt(Perm./(3792.*mio*ct.*Poro)); %%%%%%%%%%% Calculation of Hydraulic Diffusivity Value
%  SourcePoints=load('Source.txt');
%  %SourcePoints = [1 15;1 15];
%  usesecond=0;
%  usecross=1;
...
%    %F=30*ones(101);
%   % [X Y] = ndgrid(1:5, 1:5);
%    %T1 = sqrt((X-SourcePoints(1)).^2 + (Y-SourcePoints(2)).^2);
%    usesecond=0;
%    usecross=1;
%   SourcePoint = [51; 51];
